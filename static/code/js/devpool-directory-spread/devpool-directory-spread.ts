...
export function devPoolSpread() {
  fetchTotalRewards()
    .then((totalRewards) => {
      const totalRewardsElement = document.getElementById(
      ...
      const grandRewards = totalRewards.rewards.total +
        (totalRewards.lifetime?.rewardsCompletedUSD ?? 0);
      const completedAll = totalRewards.lifetime?.tasksCompletedAll ??
        totalRewards.lifetime?.tasksCompletedPriced ?? 0;
      const grandTasks = totalRewards.tasks.total + completedAll;
      const htmlBuffer = [
        `<h3>${n(grandRewards)} USD in rewards across ${
          n(grandTasks)
        } projects</h3>`,
        `
        <table id="analytics">
